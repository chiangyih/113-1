...
while 1:
    ...
    #取得frame的高度與寬度
    h,w=frame.shape[:2] #frame.shape:取得frame的高度與寬度, [:2]:取得前兩個元素(高度與寬度)
    ...
    for box in results[0].boxes.data: #results[0].boxes.data:辨識結果的boxes
        ...
        if n in ['person']: #如果辨識出來的物件名稱為person
            human_count += 1 #計算車輛數量
    ...
    cv2.putText(frame, 'Human_Count=' + str(human_count), (20,120), cv2.FONT_HERSHEY_PLAIN, 4, (0, 0, 255), 3, cv2.LINE_AA) #在畫面上顯示車輛數量
